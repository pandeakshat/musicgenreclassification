...
nav = st.sidebar.radio(
    ' ', ["Introduction", "Minor Project", "Major Project", "Conclusion"])
# endregion
# region Introduciton
if(nav == 'Introduction'):
...
if(nav == "Major Project"):
    ...
    if(options == "Introduction"):
        ...
        st.markdown(''' 
        The Project is titled, “Music Genre Classification using AI”. Music Genre Classification using AI works on the concept of Music theory, the features of an audio, the core of what makes a music and how its genres are defined, and the classification of music into different genres with the assistance of AI.
        The project is divided into multiple components that primarily work on the classification of the genre of music and using that as the foundation, we implement applications that can perform different functions and modifications on and between the music to deepen our technical understanding of audio files. The project would be outsourced as a service that can be used by the general public for purely entertainment and curious reasons, research or by content creators to enhance their workflow or for inspiration.

        The components of the project are genre classification, genre recognition, genre recommendation, genre transformation, genre creation/generation and front-end user interface.
        ''')
        st.subheader("Objectives :")
        st.write("Conversion :")
...
if(nav == 'Conclusion'):
    st.title("Conclusion")
    st.header("Thank You!")
    st.header("The Project will continously progress towards getting better with time.")
    st.header("We have launched a webApp for the same, please go their and try it out, works on desktop and mobile browsers.")
    st.subheader("Also please provide feedback on how it was and if you want something added to it in the future.")
    st.write(' ')
    st.write(' ')
    link = '[Check it Out!](https://share.streamlit.io/pandeakshat/musicgenreclassification/main/MainProject.py)'
